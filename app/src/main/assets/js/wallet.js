...
function refreshAddresses(addressesInfo){
    function truncate(item){
        var t = item.substr(0, 6) + "..." + item.substr(-7, 7);
        return t;
    }

    function setInfo(gai){ 
        ...
        for (var addr in gai){
            var addr_info  = ""
                        + " <div> "
                        + " <b>address</b>: " + truncate(gai[addr].address)
            ...
            var addrAsOption = ''
                            + '<option value="' + gai[addr].address + '">'
                                + truncate(gai[addr].address) + ' with balance [' + gai[addr].balance + ' ETH] ' 
                            + '</option>';
    ...
    web3.eth.getAccounts(function(err, addresses){
        var coinbase_text = addresses[0];
        $("#coinbase").html(coinbase_text);

        $("#transactions").html("");
        var getTransactionsLink = "" +
                            + "<h4><a href='" + b3JSI.getExplorerPrefix("/address/") + addresses[0] + "'>"
                            + "View my Transaction history</h4>"
                            + "</a></h4>";
        $("#transactions").html(getTransactionsLink);
    

        async.map(addresses, web3.eth.getBalance, function(err, balance){
            async.map(addresses, web3.eth.getTransactionCount, function(err, nonce){
                for (var i = 0; i < addresses.length; i++){
                    addressesInfo[addresses[i]] = {
                        "address": addresses[i],
                        "balance": (balance / (10**18)),
                        "nonce": nonce[0]
                    }
                    setInfo(addressesInfo);
                }
            });
        });
...
$(document).ready(function(){
    web3.eth.getAccounts(function(err, accounts){
        if (accounts.length <= 0){
            ...
            refreshAddresses(addressesInfo);
            ...
            $(".scanForAddress").click(function(e){
                e.preventDefault();
                var elemId = e.currentTarget.id;
                b3JSI.scanForAddress(elemId);
            });

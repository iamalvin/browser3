...
function refreshAddresses(){
    ...
    latest10.map(function(transaction){
        var value = web3.fromWei(transaction.value, "ether");
        var transactionData = "<div class='pure-u-1'>"
                            + " Transaction: <a href='" + b3JSI.getExplorerPrefix("/tx/") + transaction.hash + "'>" + browser3.truncateAddress(transaction.hash) + "</a><br/>"
                            + " From: " + "<a href='" + b3JSI.getExplorerPrefix("/address/") + transaction.from + "'>Coinbase</a><br/>"
                            + " To: " + "<small><a href='" + b3JSI.getExplorerPrefix("/address/") + transaction.to + "'>" + transaction.to + "</a></small></br/>"
                            + " of <b> Value: " + value + " eth. </b>"
                            + "</div>";
